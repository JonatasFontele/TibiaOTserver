...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
		if isPremium(cid) then
			selfSay('Hello ' .. creatureGetName(cid) .. '! I can take you to Venore (100gps), Ab (100gps), Carlin (100gps), Edron (100gps), Aknrahmun (100gps) or Port Hope (100gps). Where do you want to go?')
			focus = cid
			talk_start = os.clock()
		else
  	...
  	elseif focus == cid then
		...
		if msgcontains(msg, 'edron') then
			if pay(cid,100) then
				selfSay('Let\'s go!')
				selfSay('/send ' .. creatureGetName(cid) .. ', 1608 470 6')
				focus = 0
				talk_start = 0
			else
		...
		elseif msgcontains(msg, 'ab') then
			if pay(cid,100) then
				selfSay('Let\'s go!')
		...
		elseif msgcontains(msg, 'venore') then
			if pay(cid,100) then
				selfSay('Let\'s go!')
				selfSay('/send ' .. creatureGetName(cid) .. ', 1136 880 6')
				focus = 0
				talk_start = 0
			else
				selfSay('Sorry, you don\'t have enough money.')
