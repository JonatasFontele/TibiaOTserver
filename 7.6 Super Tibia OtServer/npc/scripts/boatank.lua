...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
		if isPremium(cid) then
			selfSay('Hello ' .. creatureGetName(cid) .. '! I can take you to Thais (100gps), Venore (100gps), Carlin (100gps), Ab (100gps), Edron (100gps) or Port Hope (100gps). Where do you want to go?')
			focus = cid
			talk_start = os.clock()
